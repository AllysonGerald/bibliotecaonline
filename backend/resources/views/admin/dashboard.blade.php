...
@php
    ...
    $recentRentals = \App\Models\Rental::with(['user', 'book'])->latest()->take(5)->get();
@endphp
    ...
    <div style="background: white; border-radius: 20px; padding: 32px; border: 3px solid #fbcfe8; box-shadow: 0 10px 30px rgba(236, 72, 153, 0.15);">
        ...
        @if($recentRentals->count() > 0)
            <div style="display: flex; flex-direction: column; gap: 12px;">
                @foreach($recentRentals->take(5) as $rental)
                    <a href="{{ route('admin.alugueis.show', $rental) }}" style="display: block; text-decoration: none;">
                        <div style="padding: 16px; background: linear-gradient(135deg, #faf5ff, #fdf2f8); border: 2px solid #fbcfe8; border-radius: 12px; transition: all 0.3s;" onmouseover="this.style.background='linear-gradient(135deg, #fce7f3, #fdf2f8)'; this.style.borderColor='#ec4899'; this.style.transform='translateX(4px)';" onmouseout="this.style.background='linear-gradient(135deg, #faf5ff, #fdf2f8)'; this.style.borderColor='#fbcfe8'; this.style.transform='translateX(0)';">
                            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                                <div style="flex: 1;">
                                    <p style="font-size: 14px; font-weight: 900; color: #1f2937; margin-bottom: 4px;">{{ $rental->book->titulo }}</p>
                                    <p style="font-size: 12px; color: #6b7280; font-weight: 600;">{{ $rental->user->name }}</p>
                                </div>
                                <span style="font-size: 11px; color: #9ca3af; white-space: nowrap;">{{ $rental->alugado_em->format('d/m/Y') }}</span>
                            </div>
                            <div style="display: inline-block; padding: 4px 10px; background: linear-gradient(135deg, #dbeafe, #e0f2fe); color: #0369a1; border: 2px solid #93c5fd; border-radius: 8px; font-size: 11px; font-weight: 700;">
                                {{ $rental->status->label() }}
                            </div>
                        </div>
                    </a>
