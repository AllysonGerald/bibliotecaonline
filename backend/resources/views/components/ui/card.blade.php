...
<div
    style="background: {{ $backgroundGradient }}; border-radius: 20px; padding: 32px; border: 3px solid {{ $borderColor }}; box-shadow: 0 10px 30px {{ $shadowColor }}; position: relative; overflow: hidden; {{ $class }}"
>
    ...
    <div style="position: relative; z-index: 1;">
        @if ($title || $icon)
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 24px; {{ $headerClass }}">
                @if ($icon)
        ...
        @if (isset($footer))
            <div style="margin-top: 24px; padding-top: 24px; border-top: 1px solid {{ $borderColor }}; {{ $footerClass }}">
                {{ $footer }}
            </div>
        @endif
    </div>
